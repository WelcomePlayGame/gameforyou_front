...
></div>
        ...
        <div className='crumbs_article'>
            <Breadcrumbs
            crumbs={[
                {label : `${words.BREADCRUMBS_HOME}`, url : `${process.env.PUBLIC_URL}`},
                {label : `${words.BREADCRUMBS_LIST_ARTICLE}`, url : `${process.env.PUBLIC_URL}/article`},
        ...
        <div className='statistics_article'>
            ...
            <span>Количесвто комментарий</span> 
            <span className='icons_count_box'><img src='../icons/eyes.png' className='icons_count' alt='number of pages viewed'/>{article?.statistics.action}</span>
        </div>   
        <h1 className='article_page_h1'>{article?.title}</h1>
        <div className='article_page_des'>
             <div dangerouslySetInnerHTML={typeof article?.des === 'string' ? createMarkup(article.des) : undefined}></div>
             <div className='tag_set_article'>
            <span className='tag_set_article_span'>
                {`${words.SET_TAG_FOR_ARTICLE} : `}
            {
                article?.tagSet.map((tag)=> (
                    <span key={tag.id} className='tag_set_article_span_el'>{tag.title}</span>
                ))
            }
            </span>
